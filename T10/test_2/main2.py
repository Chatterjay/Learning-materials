...
# 按年和商店分组计算总销售额
total_sales_by_year_and_store = df.groupby(["Year", "Month"])['Weekly_Sales'].sum().unstack().fillna(0)
for column in total_sales_by_year_and_store.columns:
    mean_value = total_sales_by_year_and_store[column].replace(0.00, np.nan).mean()
    total_sales_by_year_and_store[column] = total_sales_by_year_and_store[column].replace(0.00, mean_value)
...
print(total_sales_by_year_and_store.columns.astype(str).tolist())